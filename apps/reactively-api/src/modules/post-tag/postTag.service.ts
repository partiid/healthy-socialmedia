import { PartialServiceInterface } from '../../interfaces/partialService.interface';
...
export class PostTagService implements PartialServiceInterface<PostTag> {
    ...
    async create(tags: Array<number>, post: Post): Promise<boolean> {
        ...
        try {
            for (let tag of tags) {
                const tagExists = isEmpty(
                    await this.TagService.findOne({ id_tag: tag }),
                );
                if (tagExists) throw new Error('Tag not found!');

                const postTagExists = !isEmpty(
                    await this.findOne({ id_post: post.id_post, id_tag: tag }),
                );
                if (postTagExists)
                    throw new Error(
                        'Unable to add tag to post, each tag must be added exactly once.',
                    );
